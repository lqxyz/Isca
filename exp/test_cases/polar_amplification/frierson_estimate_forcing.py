...
#ALBEDO = 0.3
sst_file_name = 'sst_zonal_mean_frierson.nc'
for ALBEDO in [0.38, 0.27, 0.33]:
    print("Albedo is " + str(ALBEDO))
    exp = Experiment('frierson_albedo_'+str(ALBEDO)+'_estimate_forcing_noqflux', codebase=cb)
    exp.inputfiles = [os.path.join('input', sst_file_name)]
    exp.diag_table = diag
