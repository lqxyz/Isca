...
sst_file_name = 'sst_zonal_mean_rrtm.nc'
for ALBEDO in [0.38, 0.27, 0.33]:
    print("Albedo is " + str(ALBEDO))
    exp = Experiment('rrtm_albedo_'+str(ALBEDO)+'_estimate_forcing', codebase=cb)
    exp.inputfiles = [os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc'), os.path.join('input', sst_file_name)]
    exp.diag_table = diag
    ...
    exp.namelist = namelist = Namelist({
        ...
        'mixed_layer_nml': {
            ...
            'do_read_sst': True,
            'do_sc_sst': True,
            'sst_file': sst_file_name.replace('.nc',''),
        },
