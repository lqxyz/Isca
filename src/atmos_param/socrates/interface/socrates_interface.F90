    ...
    type(time_type) :: Time_loc
      ...
      !get ozone 
       ...
       if(do_scm_ozone) then ! Allows for option to specify ozone vertical profile in namelist for SCM.
         if(do_read_ozone) then
            call error_mesg('run_socrates', 'Cannot set do_scm_ozone and do_read_ozone = .true.', FATAL)
         endif
         if((size(temp_in,1)>1).or.(size(temp_in,2)>1)) then
            call error_mesg('run_socrates', 'Cannot set do_scm_ozone if simulating more than one column, use do_read_ozone instead', FATAL)
         endif
         if(scm_ozone(size(temp_in,3)).eq.-1) then
            call error_mesg('run_socrates', 'Input o3 must be specified on model pressure levels but not enough levels specified', FATAL)
         endif
         if(scm_ozone(size(temp_in,3)+1).ne.-1) then
            call error_mesg('run_socrates', 'Input o3 must be specified on model pressure levels but too many levels specified', FATAL)
         endif
