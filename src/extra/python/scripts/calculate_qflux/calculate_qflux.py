"""
 A method for calculating seasonally-varying qfluxes, as described in
 Russell et al 1985 DOI:10.1016/0377-0265(85)90022-3
"""
...
def qflux_calc(dataset, model_params, output_file_name, ice_file_name=None, groupby_name='months'):
    ...
    if groupby_name == 'months':
        ...
        # Have specified that var name is the same as file name as this is what the fortran assumes.
...
def upper_ocean_heat_content(dataset, model_params, time_varying_ice, dayofyear_or_months='months'):
    """Calculating upper-ocean heat content assuming a constant mixed layer depth, unlike Russel 1985,
    who have a seasonally-varying mixed layer depth.

    Note that dayofyear_or_months was designed only so that the time derrivatives of surface temperature
    could be calculated in a more time-resolved way with dayofyear data, then time-averaged onto months,
    with months used throughout the rest of the code. However, if all_time option is used, this negates
    the need for a rate-of-change calulation anyway.
    """
...
def net_surf_energy_flux(dataset, model_params, dayofyear_or_months='months'):
    """Calculates the net surface energy flux to be used in q-flux calcuation, but also calcualtes
    a scaling factor such that the annual average of the area-averaged surface flux is zero.
    """
    ...
    scaling_factor_old = (((dataset['sigma_sb_sst_clim_area_av_ocean_non_ice'] + dataset['flux_t_clim_area_av_ocean_non_ice']
                          + dataset['flux_lhe_clim_area_av_ocean_non_ice'] - dataset['flux_lw_clim_area_av_ocean_non_ice'])
                          / dataset['flux_sw_clim_area_av_ocean_non_ice'])).mean(dayofyear_or_months+'_ax')

    scaling_factor = (((dataset['sigma_sb_sst_clim_area_av_ocean_non_ice'].mean(dayofyear_or_months+'_ax')
                      + dataset['flux_t_clim_area_av_ocean_non_ice'].mean(dayofyear_or_months+'_ax')
                      + dataset['flux_lhe_clim_area_av_ocean_non_ice'].mean(dayofyear_or_months+'_ax')
                      - dataset['flux_lw_clim_area_av_ocean_non_ice'].mean(dayofyear_or_months+'_ax'))
                      / dataset['flux_sw_clim_area_av_ocean_non_ice'].mean(dayofyear_or_months+'_ax')))

    # scaling_factor = 1.0
    print('using scale factor for SW of '+ str(scaling_factor))

    net_surf_energy_fl = (scaling_factor*dataset['flux_sw_clim'] + dataset['flux_lw_clim']
                       - (model_params['sigma_sb'] * dataset['sst_clim']**4.0)
                       - dataset['flux_t_clim'] - dataset['flux_lhe_clim']) * (1.0 - dataset['land_ice_mask'])

    dataset['net_surf_energy_fl'] = ((dayofyear_or_months+'_ax','lat','lon'), net_surf_energy_fl)
    aav.area_average(dataset, 'net_surf_energy_fl', model_params,
...
def regrid_in_time(dataset, groupby_name):
    """
    Simple routine to repeat arrays in all_time-averaging situations,
    so that monthly data still output.
    """
...
def check_surface_flux_dims(dataset, model_params):
    '''
    This surface flux checker is designed to decide if we're using grey rad or not.
    If we're using grey rad then the definition of flux_sw and flux_lw are different
    to RRTM. The script was written to use RRTM output, so it changes variable names
    etc to be equivalent to RRTM definitions.
    '''

    # Rename the Socrates fluxes
    try:
        # Socrates: soc_flux_sw (time, phalf, lat, lon), Socrates net SW flux (up)
        # https://github.com/ExeClim/Isca/blob/90208a3/src/atmos_param/socrates/interface/socrates_interface.F90#L296
        dataset = dataset.rename({'soc_flux_sw':'flux_sw'})

        # Socrates: soc_flux_lw (time, phalf, lat, lon), Socrates net LW flux (up)
        # https://github.com/ExeClim/Isca/blob/90208a3/src/atmos_param/socrates/interface/socrates_interface.F90#L291
        dataset = dataset.rename({'soc_flux_lw':'flux_lw'})
    except:
        print('Not Socrates raditation scheme.')
    ...
    if 'phalf' in flux_dims_lw:
        ...
        try:
            # Script assumes flux_lw is the surface lw down (i.e. not a net flux).
            # This is the case with RRTM, but with grey radiation 'flux_lw' is the net lw flux in 3D.
            # So we take the lwdn_sfc output from grey rad and rename it flux_lw.
            dataset['lwdn_sfc']
            dataset = dataset.rename({'lwdn_sfc':'flux_lw'})
        except:
            # If lwdn_sfc is not available, then we re-calculate it from flux_lw by
            # adding back sigma*t_surf**4, then call it flux_lw
            print('lwdn_sfc not present when using grey radiation, so re-calculating it from flux_lw.')
            max_pressure = dataset.phalf.max()
            lwdn_sfc = -dataset.flux_lw_3d.sel(phalf=max_pressure) + model_params['sigma_sb'] * dataset.t_surf**4.
            new_dims = ('time','lat','lon')
...
if __name__ == "__main__":
    ...
    # Folder containing the python script and input files that ran the experiment
    base_exp_name = 'annual_mean_ice_post_princeton_fixed_sst/'
    # Folder within the data directory where the files can be found
    exp_name = 'annual_mean_ice_post_princeton_fixed_sst_1'
    ...
    # Proposed name of your output qflux file. Will also be qflux field name in q-flux netcdf file
    # as the fortran assumes file-name = field name. No need to add '.nc' or any file paths in this
    # variable as otherwise they will end up in the field name too. Output file will be stored in
    # the same directory as this script.
    output_file_name = 'ami_test_interp'

    start_file = 240
    end_file = 360
    land_present = True
    # Conditions the script on whether to expect data on sigma levels (if False)
    # or pressure levels (if True).
    # Script should be insensitive to this choice if both sets of files exist.
    use_interpolated_pressure_level_data = False
    # Set time increments of input files (e.g. `monthly` for `atmos_monthly` files.
    avg_or_daily = 'monthly'

    # Set the time frequency of output data.
    # Valid options are 'months', 'all_time' or 'dayofyear'.
